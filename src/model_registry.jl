...
function load_model(name::String; dir::String = models_dir())
    ...
    return CustomMLP(
        ...
        nn,
        convert(Int, d["batch_size"]),
        relu,                                      # activation: always relu
        convert(String, d["device"]),
        convert(Bool, d["shuffle"]),
        convert(String, d["optimizer"]),
        Float32(d["learning_rate"]),
        convert(Int, d["nepochs"]),
        Float32(d["beta1"]),
        Float32(d["beta2"]),
        Float32(d["weight_decay"]),
        Float32(d["momentum"]),
