...
function mb_inputs(mb_model::CustomMLP)
    ...
    if any(==("slope"), mb_model.input_features)
        inputs = (;
            inputs...,
            slope = Sleipnir.iTopoSlope(window_m = topography_window_m(mb_model)),
        )
    end
    if any(==("aspect"), mb_model.input_features)
        inputs = (;
            inputs...,
            aspect = Sleipnir.iTopoAspect(window_m = topography_window_m(mb_model)),
        )
    end
    return inputs
end
...
function _feature_values(climate_2D_period::Climate2Dstep, feature::AbstractString)
...
function _build_feature_matrix(mb_model::CustomMLP, climate_2D_period::Climate2Dstep)
    ...
    for (idx, feature) in enumerate(mb_model.input_features)
        ...
        normalized_values =
            mb_model.norm === nothing ? Float32.(values) :
